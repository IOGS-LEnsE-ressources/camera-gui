function varargout = Zygozago2(varargin)

% Last Modified by GUIDE v2.5 10-Apr-2007 10:27:13
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @Zygozago2_OpeningFcn, ...
    'gui_OutputFcn',  @Zygozago2_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
%--------------------------------------------------------------------------
function Zygozago2_OpeningFcn(hObject, eventdata, handles, varargin)
% Choose default command line output for Zygozago2
...
% Update handles structure
guidata(hObject, handles);
% --- Executes just before Zygozago2 is made visible.
function Zygozago2_CreateFcn(hObject, eventdata, handles, varargin)
...
function varargout = Zygozago2_OutputFcn(hObject, eventdata, handles)
...
if(~isempty(handles.surface_absoustr))
    h = figure;
...
switch handles.typeplot
    case 1
        ...
        grph=mesh(x_image,y_image,phi*handles.unit);set(grph,'ButtonDownFcn',{@image_surface_ButtonDownFcn, handles});view(-45,60)
        set(handles.image_surface,'Color','black','XColor','white', 'YColor','white', 'ZColor','white');
    case 2
        axis(handles.image_surface,'auto');
        [C,h]=contour(phi*handles.unit);,axis ij,axis image, colorbar;set(h,'HitTestArea','on');set(h,'ButtonDownFcn',{@image_surface_ButtonDownFcn, handles});
        set(handles.image_surface,'Color','black','XColor','white', 'YColor','white', 'ZColor','white');
    case 3
...
switch val
    ...
    case 2
        ...
        [C,h]=contour(phi,20);axis xy,axis equal,colorbar;set(h,'HitTestArea','on');set(h,'ButtonDownFcn',{@image_surface_ButtonDownFcn, handles});
