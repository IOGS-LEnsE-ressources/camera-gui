...
% In R2014a Student Version we want to let users on Korean machines know
% that they can enable Korean localization.
if isstudent && (ismac || ispc)
    localeStruct = feature('locale');
    if strncmp(localeStruct.messages,'en',2) && strncmp(localeStruct.ctype,'ko_KR',5)
        disp(getString(message('MATLAB:matlabrckoKRSV:EnableKorean')));
    end
end
