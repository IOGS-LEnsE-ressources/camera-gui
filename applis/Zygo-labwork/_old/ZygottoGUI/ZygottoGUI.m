function varargout = ZygottoGUI(varargin)

% Last Modified by GUIDE v2.5 22-Jul:-2016 09:46:07
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @ZygottoGUI_OpeningFcn, ...
    'gui_OutputFcn',  @ZygottoGUI_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
%--------------------------------------------------------------------------
function ZygottoGUI_OpeningFcn(hObject, eventdata, handles, varargin)
global o;
o=0;
% Choose default command line output for ZygottoGUI
...
switch a
    ...
    case 1
        handles.unit = 632.8;
        set(handles.radio_config_nano,'Value',0);handles.h.SaveImage('Caméra.png'); % on enregistre l'image sur le disque dur
...
save_config('gamma',1.5);
handles.gamma = read_config('gamma',1);
...
if(handles.typeacq==1)
    ...
    %     handles.h=actxcontrol('UEYECAM.uEyeCamCtrl.1','position',[300 338 235 235]); % on crée l'objet pour la caméra
    %     handles.h.InitCamera(0);
    %     handles.h.SaveImage('Caméra.png'); % on enregistre l'image sur le disque dur
    %     img=double(imread('Caméra.png'));
    %
    %     handles.img_interf_NB = repmat(img/max(max(img)),[1,1,3]);
    %     handles.img1=[];
    %     handles.img2=[];
...
% Update handles structure
guidata(hObject, handles);
% --- Executes just before ZygottoGUI is made visible.
function ZygoGUI_CreateFcn(hObject, eventdata, handles, varargin)
...
function varargout = ZygottoGUI_OutputFcn(hObject, eventdata, handles)
...
if(~isempty(handles.img_interf_NB))
    h = figure('name','Tracer une ligne de longueur connue, par clique gauche puis droit');
...
handles.etude = 0;
set(handles.radio_config_aberrations,'Value',0);
panel_surface=findobj('Tag','panel_infos_surface');
panel_aberrations=findobj('Tag','panel_infos_aberrations');
panel_NF=findobj('Tag','panel_infos_NF');
set(panel_surface,'Visible','on');
...
handles.etude = 1;
set(handles.radio_config_surface,'Value',0);
panel_surface=findobj('Tag','panel_infos_surface');
panel_aberrations=findobj('Tag','panel_infos_aberrations');
panel_NF=findobj('Tag','panel_infos_NF');
set(panel_surface,'Visible','off');
...
handles.h.SaveImage('Masque.png'); % on enregistre l'image sur le disque dur
img=double(imread('Masque.png'));
...
save_config('Tensions', [0.8 1.6 2.4 3.2 4]);
...
switch handles.typeacq
    case 0
        ...
        N = 5 ; % On définit le nombre d'images pour le calcul de mesure de phase shift
        handles.h.SaveImage('Caméra1.png'); % on enregistre l'image sur le disque dur
        sizeimg=double(imread('Caméra1.png'));
        save_config('Tensions', [0.81 2*0.81 3*0.81 4*0.81 5*0.81]);
        tensions_commande=read_config('Tensions',1);
...
switch handles.typeplot
    case 1
        ...
        grph=mesh(x_image,y_image,flipud(phi)*handles.unit);set(grph,'ButtonDownFcn',{@ButtonDownFcn, handles});view(-45,60)
        set(handles.image_surface,'Color','black','XColor','white', 'YColor','white', 'ZColor','white');
    case 2
        % axis(handles.image_surface,'auto');
        [C,h]=contour(flipud(phi)*handles.unit);,axis xy,axis tight, colorbar;set(h,'HitTestArea','on');set(h,'ButtonDownFcn',{@ButtonDownFcn, handles});
        set(handles.image_surface,'Color','black','XColor','white', 'YColor','white', 'ZColor','white');
    case 3
        grph=imagesc(x_image,y_image,phi*handles.unit);axis(handles.image_surface,'tight');
...
switch handles.typeplot
    case {1}
        ...
        title(Titre_text)
    case {2}
        contour(x_image,y_image,flipud(phi),handles.nbcont);,axis ij,axis tight, colorbar;
        title(Titre_text)
    case {3}
...
switch val
    ...
    case 2
        ...
        [C,h]=contour(x_image,y_image,flipud(phi),20);,axis tight,colorbar;set(h,'HitTestArea','on');set(h,'ButtonDownFcn',{@image_surface_ButtonDownFcn, handles});
    ...
    case 3
        ...
        if ~(isequal(phi,[]))
            view(2);
            imagesc(x_image,y_image,phi*handles.unit),axis tight;
            [px,py,p,xi,yi] = improfile;
            handles.profile_xi=xi;
            handles.profile_yi=yi;
...
if ~(isequal(handles.surface_absoustr,[]))
    ...
    if(handles.typeplot==2)
        ...
        set(h,'HitTestArea','on'), axis ij,axis tight, colorbar;set(h,'ButtonDownFcn',{@image_surface_ButtonDownFcn, handles});
        set(handles.image_surface,'Color','black','XColor','white', 'YColor','white', 'ZColor','white');
    end
end
guidata(hObject, handles);
...
switch handles.typeplot
    ...
    case 2
        ...
        [C,h]=contour(x_image,y_image,flipud(phi)*handles.unit,handles.nbcont);,axis tight,colorbar;
    ...
    case 4
        set(handles.image_surface,'Color','black','XColor','white', 'YColor','white', 'ZColor','white');
        grph=imagesc(x_image,y_image,phi*handles.unit),axis tight;
end
guidata(hObject, handles);
...
if(~get(hObject,'Value'))
    ...
    set(handles.radio_config_images,'Value',0);
    handles.typeacq = 0;
    save_config('typeacq',handles.typeacq);
    guidata(hObject, handles);
end
...
% --- Executes during object creation, after setting all properties.
function edit_config_f_CreateFcn(hObject, eventdata, handles)
% hObject    handle to edit_config_f (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes when selected object is changed in buttongroup_config_type.
function buttongroup_config_type_SelectionChangeFcn(hObject, eventdata, handles)
% hObject    handle to the selected object in buttongroup_config_type
...
% --- Executes during object creation, after setting all properties.
function button_menu_mesure_CreateFcn(hObject, eventdata, handles)
% hObject    handle to button_menu_mesure (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
switch (o)
    case (0)
        [handles.h,truc]=actxcontrol('UEYECAM.uEyeCamCtrl.1'); % on crée l'objet pour la caméra
        set(truc,'units','normalized');
        set(truc,'position',[0.24 0.415*0.69+0.24 0.25 0.37]);
        handles.h.InitCamera(0);
        set(handles.Initialisation,'String','Paramètres Caméra');
...
global o;
o=0;
set(handles.Initialisation,'String','Initialisation');
