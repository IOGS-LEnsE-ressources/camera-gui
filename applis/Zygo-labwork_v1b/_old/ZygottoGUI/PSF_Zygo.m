...
while  re_calcul_tout==0;
    ...
    PSF_norm = PSF_norm/(max(max(PSF_parfaite))) ; %Normalisation � la tache d'airy
    ...
    while  retourne_menuP==0;
        ...
        menu_PSF=menu('Menu PSF','Niveau de gris','Coupe X',...
            'Coupe Y','Defoc. (ecart_normal)','Defoc. (delta Z)','Energie encercl�e','Choix �chantillonage','Fermer Menu PSF');
        ...
        if menu_PSF==4
            fig_psf5= figure('name','PSF avec d�focalistion','position',[100 200 1000 400]);
            for Num_defoc=1:5
                ...
                PSF_norm_defocalise = PSF_norm_defocalise/(max(max(PSF_parfaite))) ; %Normalisation � la tache d'airy
                PSF_parfaite = PSF_parfaite/(max(max(PSF_parfaite))) ;
            ...
            title(['1/4 Lambda, soit :' num2str(delta_Z,2) 'mm de d�foc. entre plans images'])
        ...
        if menu_PSF==5
            ...
            fig_psf6= figure('name','PSF avec d�focalistion','position',[100 200 1000 400]);
            ...
            title([num2str(delta_Z/2,2) 'mm de d�foc. entre plans images, soit un ecart normal :' num2str(ecart_normal/2,2) 'lambda'])
        ...
        if menu_PSF==7
            ...
            prompt  = {'Nombre d''�chantillons, Ne :128, 256, 512 ou 1024','Zoom de la PSF : 1, 2 , 3 , 4 ou 5 '};%
            %Rapport plan pupille = 2^zoom
            val_echant    =    val_def_echant;
            titre_dialogue   = 'Configuration de l''�chantillonnage';
            lineNo  = 1;
