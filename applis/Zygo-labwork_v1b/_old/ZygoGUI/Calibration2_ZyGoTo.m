...
switch (o)
    case (0)
handles.h=actxcontrol('UEYECAM.uEyeCamCtrl.1','position',[30 30 300 300]); % on cr�e l'objet pour la cam�ra
handles.h.InitCamera(0);
set(handles.Bouton_Initialisation,'String','Param�tres Cam�ra');
...
handles.h.SaveImage('Cam�ra.png'); % on enregistre l'image sur le disque dur
a=double(imread('Cam�ra.png'));
axes(handles.axes1);
imagesc(a),colorbar;
...
dephasages=180*asin(valeurs_moy/Am)/pi ; %en degr� phi/2
 
%figure,  plot(dephasages);

der_deph=diff(dephasages) ; % on calcul la derivee
i=find(0>der_deph) ; % points � derivee negative 
