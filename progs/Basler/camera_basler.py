"""CameraBasler class to communicate with a Basler camera sensor.
...
def get_converter_mode(color_mode: str) -> int:
    """
    Returns the corresponding converter display mode

    :param color_mode: color mode of the camera ('Mono8', 'Mono10', 'Mono12' or 'RGB8')
    :type color_mode: str    
...
class CameraBasler():
    ...
    :param camera_device: Device object from the pypylon wraper.
    :type camera_device: pylon.TlFactory.device
    ...
    def __init__(self, cam_dev: pylon.TlFactory) -> None:    
        ...
        self.camera = pylon.InstantCamera(tlFactory.CreateDevice(self.camera_device))   # ex camera !!
        # Goal of this object .???????????????????????????????????????
    ...
    def set_display_mode(self, colormode: str = 'Mono8') -> None:
        """
        Change the color mode of the displayed image from the camera
    ...
    def get_color_mode(self):
        """
        Get the colormode of acquisition of the device
    ...
    def set_color_mode(self, colormode: str) -> None:
        """
        Change the colormode of acquisition of the device
    ...
    def set_aoi(self, x0, y0, w, h) -> bool:
        """
        Set the area of interest (aoi) for capture an image.
    ...
    def get_aoi(self) -> tuple[int, int, int, int]:        
        """
        Return the area of interest (aoi) for capture an image.

        :return: [x0, y0, width, height] x0 and y0 are the coordinates of the top-left corner and width and height are the size of the aoi.
    ...
    def reset_aoi(self) -> None:
        """
        Reset the area of interest (aoi) to its maximum size
        """
        self.aoi_x0 = 0
        self.aoi_y0 = 0
        self.aoi_width = self.width_max
        self.aoi_height = self.height_max
        print(self.set_aoi(self.aoi_x0, self.aoi_y0, self.width_max, self.height_max))
    ...
    def get_exposure_range(self) -> tuple[float, float]:
        """
        Return the minimum and the maximum value of the exposure time in microseconds.
    ...
    def get_frame_rate_range(self):
        """
        Return the minimum and the maximum value of the frame rate in frames per second.
    ...
    def get_black_level_range(self) -> tuple[int, int]:
        """
        Return the minimum and the maximum value of the black level.
