 ...
 * \version 1.1.1
 *
 * Copyright (C) 2020 - 2024, IDS Imaging Development Systems GmbH.
 *
    ...
    unsigned int errorCounter, int incomplete, int dropped, int lost, bool showCustomNodes)
{
    ...
    if (showCustomNodes)
    {
        strText = QString("Framerate: %1, conversion: %2 ms, frames acquired: %3, errors: %4, incomplete: %5, dropped: %6, lost: %7")
            .arg(QString::number(m_frameRate, 'f', 1), QString::number(m_conversionTime_ms, 'f', 1), QString::number(frameCounter), 
                QString::number(errorCounter), QString::number(incomplete), QString::number(dropped), QString::number(lost));
    }
    else
    {
        strText = QString("Framerate: %1, conversion: %2 ms, frames acquired: %3, errors: %4")
            .arg(QString::number(m_frameRate, 'f', 1), QString::number(m_conversionTime_ms, 'f', 1), QString::number(frameCounter), 
