...
{
    std::cout << "IDS peak genericAPI Walkthrough Sample v" << VERSION << std::endl;
     ...
     * In addition the try scope makes sure that the instances of IDS peak genericAPI objects will be automatically cleaned
    ...
    try
    {
        ...
        if (cti_selection.empty())
        {
             ...
             *
             * The IDS peak genericAPI's EnvironmentInspector provides the function CollectCTIPaths() which evaluates this
         ...
         *
         * The IDS peak genericAPI provides two classes for each of the GenTL module entities.
         ...
         *          DataStreams() list.
         *
         * To simplify the above procedure up to step 7, the IDS peak genericAPI provides the class DeviceManager.
...
#pragma region open_the_first_available_device
         ...
         *
         * Note: In the IDS peak genericAPI the Local Device is referred to as 'Device'
        ...
        try
        {
            ...
            try
            {
                ...
                if (current_trigger_selector_setting != "AcquisitionStart")
                {
                    ...
                    {
                        std::cout << "INFO: Setting the remote device node 'TriggerSelector' to value "
                                     "'AcquisitionStart' not possible. There is no such entry."
                                  << std::endl;
                        std::cout << "      Msg: " << e.what() << std::endl;
                    }
                    ...
                    {
                        std::cout << "INFO: Setting the remote device node 'TriggerSelector' to value "
                                     "'AcquisitionStart' not possible. 'TriggerSelector' might not be writeable or "
                                     "'AcquisitionStart' might not be available."
                                  << std::endl;
                        std::cout << "      Msg: " << e.what() << std::endl;
                    }
                    catch (const std::exception& e)
