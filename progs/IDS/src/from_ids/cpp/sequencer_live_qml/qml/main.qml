...
ApplicationWindow {
    ...
    Connections {
        ...
        onStartupFinished: {
            if (!imageWindow) {
                ...
                imageWindow.closing.connect(onImageWindowDestroyed)
            } else {
                imageWindow.show()
            }
    ...
    function onImageWindowDestroyed() {
        root.close()
    }
            ...
            PropertyContainer {
                ...
                content: [
                    PropertyControl_FloatSlider {
                        ...
                        decimals: 0
                        decimalFactor: 1
                        fallbackValue: "Exposure Time"
                    },
                    PropertyControl_EnumComboBox {
                        id: gainSelectorControl
                        width: parent.width - 2 * parent.padding
                        fallbackValue: "Gain Selector"
                    },
                    PropertyControl_FloatSlider {
                        id: gainControl
                        width: parent.width - 2 * parent.padding
            ...
            PropertyContainer {
                ...
                content: [
                    PropertyControl_CommandButton {
                        ...
                        width: parent.width - 2 * parent.padding
                        dirty: spinBox.dirty
                        fallbackValue: "Save Sequencer Set"
                        onExecuted: {
                    ...
                    PropertyControl_CommandButton {
                        ...
                        autoApply: false
                        width: parent.width - 2 * parent.padding
                        fallbackValue: "Load Sequencer Set"
                        onExecuted: {
    ...
    Dialog_Error {
        ...
        onAccepted: {
            if (root && root.imageWindow) {
                root.imageWindow.destroy()
            }

            if(roiSizeDialog)
            {
                roiSizeDialog.destroy()
            }

            if(triggerDialog)
            {
                triggerDialog.destroy()
            }
            ...
            offsetXControl.enabled = false
            offsetYControl.enabled = false
            gainSelectorControl.enabled = false
            spinBox.enabled = false
            runSequencerButton.enabled = false
            saveControl.node = null
            loadControl.node = null
    ...
    Dialog {
        ...
        Text {
            ...
            text: "This program uses Qt version " + (backEnd ? backEnd.QtVersion() : 0)
    ...
    Text {
        id: versionText
        width: 200
        text: backEnd ? backEnd.Version() : 0
        horizontalAlignment: Text.AlignRight
        verticalAlignment: Text.AlignVCenter
