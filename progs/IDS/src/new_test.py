    ...
    try:
        ...
        for i in range(device_count):
            if device_manager.Devices()[i].IsOpenable():
                ...
                m_node_map_remote_device.FindNode("TriggerSelector").SetCurrentEntry("ExposureStart")
                m_node_map_remote_device.FindNode("TriggerSource").SetCurrentEntry("Software")
                m_node_map_remote_device.FindNode("TriggerMode").SetCurrentEntry("On")

                m_node_map_remote_device.FindNode("PixelFormat").SetCurrentEntry("Mono8")
                return True
...
def start_acquisition():
    try:
        m_dataStream.StartAcquisition(peak.AcquisitionStartMode_Default)
...
def get_image():
    try:
        # trigger image
        m_node_map_remote_device.FindNode("TriggerSoftware").Execute()
        buffer = m_dataStream.WaitForFinishedBuffer(200)
        # convert to RGB
